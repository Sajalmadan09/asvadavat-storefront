...
export function SiteHeader() {
  ...
  return (
    <>
      ...
      <div className="relative hidden w-full overflow-hidden border-b border-[#d6e9dc] bg-white md:block">
      ...
      <nav className="sticky top-0 z-40 hidden items-center justify-center gap-1 border-b border-[#d6e9dc] bg-white py-2 shadow-sm md:flex">
        {NAV_LINKS.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="group relative rounded-lg px-4 py-2 text-sm font-medium tracking-wide text-[#1d3a2c] transition-colors hover:text-[#3f8c63]"
          >
            {link.label}
            <span className="absolute inset-x-2 -bottom-0.5 h-0.5 origin-left scale-x-0 rounded-full bg-[#3f8c63] transition-transform group-hover:scale-x-100" />
          </Link>
        ))}
        <span className="mx-1 text-[#c7decf]">|</span>
        <Link
          href="/enquiry"
          className="relative rounded-full bg-gradient-to-r from-[#4f9d73] to-[#3b805d] px-6 py-2 text-sm font-bold text-white shadow transition-transform hover:scale-105 active:scale-95"
        >
          Enquiry
          {count > 0 && (
        ...
        <button
          ...
          className="ml-2 flex h-8 w-8 items-center justify-center rounded-full border border-[#d6e9dc] text-sm font-bold text-[#3f8c63] transition-colors hover:bg-[#eaf6ee]"
      ...
      <div className="sticky top-0 z-40 border-b border-[#e8e0d0] bg-white shadow-sm md:hidden">
        <div className="relative flex items-center justify-between px-3 py-3.5">
          ...
          <Link
            href="/products"
            className="relative z-10 flex items-center gap-1 rounded-full bg-gradient-to-r from-[#2d6a4f] to-[#1b4332] px-3.5 py-2 text-xs font-bold text-white shadow active:scale-95"
          >
      ...
      {/* ── Mobile drawer ── */}
      {menuOpen && (
        <div className="sticky top-[5.25rem] z-40 animate-slide-down border-b border-[#e8e0d0] bg-white px-4 pb-5 pt-2 shadow-md md:hidden">
