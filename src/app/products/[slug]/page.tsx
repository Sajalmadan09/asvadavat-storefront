  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-6 md:py-8">
      ...
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-olive-500 md:text-sm">
        <Link href="/products" className="font-medium hover:text-[#3f8c63]">Products</Link>
        <span>&rsaquo;</span>
        <Link href={`/products?category=${product.category}`} className="font-medium hover:text-[#3f8c63]">{categoryLabels[product.category]}</Link>
        <span>&rsaquo;</span>
        <span className="truncate text-[#2b503a]">{product.nameEn}</span>
      </nav>

      <div className="grid gap-5 md:grid-cols-2 md:gap-8">
        <section className="overflow-hidden rounded-2xl border border-[#dff0e6] bg-white shadow-sm">
          <ProductCarousel
          ...
          <div className="p-5">
            ...
            <h1 className="mt-2 text-2xl font-extrabold text-[#1d3a2c] md:text-3xl">{product.nameEn}</h1>
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {product.tags.map((tag) => (
                <span key={tag} className="rounded-md bg-[#ecf8f1] px-2 py-0.5 text-xs font-medium text-[#3f8c63]">#{tag}</span>
              ))}
            </div>
            <div className="mt-5 rounded-xl bg-[#ecf8f1] p-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#3f8c63]">All Pack Sizes</h3>
              <div className="mt-2 grid grid-cols-2 gap-2">
                {product.prices.map((price) => (
                  <div key={`${product.id}-${price.packSize}`} className="rounded-lg border border-[#cde6d7] bg-white px-3 py-2 text-center">
                    <p className="text-xs font-medium text-olive-500">{price.packSize}</p>
                    <p className="text-base font-extrabold text-[#1d3a2c]">Rs.&nbsp;{price.netPriceInr}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Link href="/products" className="block rounded-xl border border-[#cde6d7] bg-white px-4 py-3 text-center text-sm font-bold text-[#37684b] shadow-sm transition-colors hover:bg-[#f0f8f3] active:bg-[#e4f2ea]">
